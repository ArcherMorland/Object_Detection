...
def fill_truth_detection(lab_valueset, img_size, crop, flip, dx, dy, sx, sy):
    max_boxes = 50
    ...
    if True:
        ...
        for i in range(bs.shape[0]):
            ...
            y2 = bs[i][2] + bs[i][4]/2'''
            x1 = bs[i][1]/img_width
            y1 = bs[i][2]/img_height
            x2 = bs[i][3]/img_width
            y2 = bs[i][4]/img_height
            #print('cp4 x1:   ',x1)
            #print('cp4 y1:   ',y1)
            #print('cp4 x2:   ',x2)
            #print('cp4 y2:   ',y2)
...
def load_data_detection(imgpath, shape, crop, jitter, hue, saturation, exposure, labset):
    ...
    label = fill_truth_detection(labset, original_img_size, crop, flip, -dx, -dy, sx, sy)
    #print('cp3',img.size, label.shape)
    return img, label
